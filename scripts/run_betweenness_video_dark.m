%% RUN_BETWEENNESS_VIDEO_DARK
%
% Dark-theme variant of RUN_BETWEENNESS_VIDEO, kept in a separate script so
% the original (rotating-frame-on-white) videos are untouched. Both panels
% now share a true-black background; colours, marker edges, and the node
% diagram are re-tuned for contrast against black instead of white.
%
% Layout:
%   Left  — synodic (rotating) frame  with dark cislunar background
...
TU_days = cfg.units.TU_days; %#ok<NASGU>

% Colour palette — brightened versions of the light-theme colours for
% contrast against black (matches run_betweenness_dc_visuals_dark)
c_A  = [0.30 0.55 1.00];   % origin — bright blue
c_Br = [0.35 0.85 0.45];   % bridge — bright green
c_B  = [0.95 0.35 0.20];   % dest   — bright red-orange

BG_ROT = [0.0 0.0 0.0];   % black — rotating frame background
BG_ECI = [0.0 0.0 0.0];   % black — ECI frame background
TXTC   = [0.92 0.94 0.99];
EDGEC  = [0.90 0.92 0.97]; % light marker-edge for contrast on black

% ── Scan for *_dc.mat files ───────────────────────────────────────────────────
if ~exist(EXPLAINER_DIR, 'dir')
    error('[video_dark] Explainer folder not found:\n  %s', EXPLAINER_DIR);
end
...
if isempty(mat_files)
    error('[video_dark] No *_dc.mat files found under:\n  %s', EXPLAINER_DIR);
end
fprintf('[video_dark] Found %d dc mat(s).\n\n', numel(mat_files));
...
for fi = 1:numel(mat_files)
    ...
    % ECI: centred on Earth (0,0), square limits from max trajectory extent
    half_eci = max(max(abs(path_x_eci)) + pad, max(abs(path_y_eci)) + pad);
    eci_xlim = [-half_eci, half_eci];
    eci_ylim = [-half_eci, half_eci];
    ...
    % ── Build figure ──────────────────────────────────────────────────────────
    fig = figure('Color', BG_ROT, 'Visible', 'off', 'InvertHardcopy', 'off', ...
                 'Units', 'pixels', 'Position', [0 0 1600 800]);

    ax_rot  = axes('Parent', fig, 'Position', [0.03  0.07  0.44  0.88]);
    ax_eci  = axes('Parent', fig, 'Position', [0.535 0.07  0.44  0.88]);
    ax_node = axes('Parent', fig, 'Position', [0.40  0.84  0.20  0.14]);

    % Panel labels (below each panel, on the black figure background)
    annotation(fig, 'textbox', [0.03,  0.01, 0.44, 0.05], ...
        'String', 'Rotating Frame', ...
        'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
        'EdgeColor', 'none', 'BackgroundColor', 'none', ...
        'FontSize', 11, 'Color', TXTC, 'FitBoxToText', 'off');
    annotation(fig, 'textbox', [0.535, 0.01, 0.44, 0.05], ...
        'String', 'Earth-Centered Inertial', ...
        'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
        'EdgeColor', 'none', 'BackgroundColor', 'none', ...
        'FontSize', 11, 'Color', TXTC, 'FitBoxToText', 'off');
    ...
    cr3bp_plot_background_dark(CJbg, mu, ax_rot);
    ...
    % Earth and Moon in rotating frame (fixed) — overlaid bigger than the
    % background's true-scale glow circles for video legibility
    plot(ax_rot, -mu,   0, 'o', 'MarkerFaceColor', [0.20 0.47 0.90], ...
         'MarkerEdgeColor', EDGEC, 'MarkerSize', 11, 'HandleVisibility', 'off');
    plot(ax_rot, 1-mu,  0, 'o', 'MarkerFaceColor', [0.68 0.68 0.68], ...
         'MarkerEdgeColor', EDGEC, 'MarkerSize', 7,  'HandleVisibility', 'off');

    % POs (dashed)
    local_plot_po(ax_rot, XpoA,  c_A);
    local_plot_po(ax_rot, XpoBr, c_Br);
    local_plot_po(ax_rot, XpoB,  c_B);

    % Updateable trail + dot
    h_hist_rot  = plot(ax_rot, NaN, NaN, '-',  'Color', [0.32 0.34 0.40], 'LineWidth', 0.6);
    ...
    h_dot_rot = plot(ax_rot, NaN, NaN, 'o', 'MarkerSize', 9, ...
        'MarkerFaceColor', 'w', 'MarkerEdgeColor', EDGEC, 'LineWidth', 1.5);

    % ── Static ECI frame ─────────────────────────────────────────────────────
    set(ax_eci, 'Color', BG_ECI, 'XColor', [0.40 0.42 0.50], 'YColor', [0.40 0.42 0.50]);
    hold(ax_eci, 'on');
    ...
    plot(ax_eci, cos(theta_c), sin(theta_c), '-', ...
         'Color', [0.30 0.33 0.43], 'LineWidth', 0.9);   % Moon orbit
    ...
    h_hist_eci  = plot(ax_eci, NaN, NaN, '-', 'Color', [0.22 0.24 0.34], 'LineWidth', 0.6);
    ...
    % ── Video writer ──────────────────────────────────────────────────────────
    vid_path = fullfile(ex_dir, [base_name '_video_dark.mp4']);
...
fprintf('[video_dark] All examples complete.\n');
...
% Draws the 3-node relay diagram.  Returns handles for dynamic highlighting.
    hold(ax, 'on');
    % DataAspectRatio=[1 1 1] ensures fill circles in data space are circular on screen
    ...
    for k = 1:3
        h_nc(k) = fill(ax, cx(k) + r*cos(th), cy(k) + r*sin(th), colors{k}, ...
            'EdgeColor', [0.85 0.87 0.92], 'LineWidth', 1.5);
    ...
    h_na(1) = quiver(ax, cx(1)+gap, cy(1), cx(2)-cx(1)-2*gap, 0, 0, ...
        'Color', [0.90 0.92 0.97], 'LineWidth', 2.0, 'MaxHeadSize', 0.8, 'AutoScale', 'off');
    h_na(2) = quiver(ax, cx(2)+gap, cy(2), cx(3)-cx(2)-2*gap, 0, 0, ...
        'Color', [0.90 0.92 0.97], 'LineWidth', 2.0, 'MaxHeadSize', 0.8, 'AutoScale', 'off');

    % Thin border
    plot(ax, [0 1 1 0 0], [0.25 0.25 0.75 0.75 0.25], '-', ...
         'Color', [0.40 0.44 0.52], 'LineWidth', 0.8);
end

% ─────────────────────────────────────────────────────────────────────────────

function local_update_node_diagram(h_nc, h_na, phase, c_A, c_Br, c_B)
% "Dim" now blends toward black (receding into the dark background) instead
% of toward white, so de-emphasized nodes read as dimmer rather than paler.
    dim = @(c) 0.35*c + 0.65*[0 0 0];
    COL_DIM_ARROW = [0.40 0.42 0.48];
    COL_ACT_ARROW = [0.92 0.94 0.99];
