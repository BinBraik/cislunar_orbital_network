...
% ── CACHE DIRECTORY ─────────────────────────────────────────────────
CACHE_DIR = '';   % '' = default (repo/atlas_cache)
...
if isempty(CACHE_DIR)
    CACHE_DIR = fullfile(repo_root(), 'atlas_cache');
end
...
entries = atlas_cache_inspect(CACHE_DIR, true);
...
for ei = 1:numel(candidate_entries)
    ...
    sub_fp  = atlas_cache_fingerprint(fam, e_src.parsed.mu, e_src.parsed.CJ, cfg_sub);
    ...
    info_sub = atlas_cache_get_path(fam, e_src.parsed.mu, e_src.parsed.CJ, cfg_sub);
    ...
    S_src = tmp.S;
    S_src.grid3 = atlas_grid_make(cfg_src);   % rebuild function handles

    % Derive subset
    try
        S_sub = atlas_derive_subset(S_src, cfg_sub);
    catch ME
    ...
    % Save
    info = atlas_cache_save(S_sub, cfg_sub);
...
if nDerived > 0
    ...
    fprintf('%s\n', repmat('─', 1, 72));
    fprintf('  PASTE THIS INTO run_overlap_all_pairs.m / run_overlap_pair_visuals.m:\n');
    fprintf('%s\n', repmat('─', 1, 72));

    % Build a temporary parsed struct for the sub-config using representative
    p = atlas_cache_fingerprint_parse( ...
            atlas_cache_fingerprint(src_rep.family, src_rep.parsed.mu, ...
