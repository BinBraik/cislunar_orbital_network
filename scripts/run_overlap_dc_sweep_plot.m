...
% Replay and save plots from a completed dc_sweep run.
%
% Reads dc_sweep_results.mat (produced by run_overlap_dc_sweep.m) and calls
% traj_diffcorr_visualize for each selected pair, saving a side-by-side
% before/after PNG plus a plain-text info file per pair and a summary CSV.
%
% USER SETTINGS
%   RESULTS_FILE  : full path to dc_sweep_results.mat
...
%
% Outputs  (written to <run_root>/dc_sweep_plots/)
...
if isempty(RESULTS_FILE)
    candidates = dir(fullfile(repoRoot, 'atlas_results', '*', ...
                              'dc_sweep', 'dc_sweep_results.mat'));
    if isempty(candidates)
        error('[dc_plot] Cannot find dc_sweep_results.mat. Set RESULTS_FILE manually.');
    end
...
% =========================================================================
% OUTPUT FOLDER  (sibling to dc_sweep/)
% =========================================================================
dcRoot    = fileparts(RESULTS_FILE);
plotsRoot = fullfile(fileparts(dcRoot), 'dc_sweep_plots');
...
for k = 1:nFam
    [family_list{k}, ~] = atlas_prepare_or_load(families{k}, cfg, grid3);
...
for kp = 1:numel(plot_idx)
    ...
    pairSafe = sanitize_fname(pairTag);
    ...
    try
        traj_diffcorr_visualize(T, Tc, SA, SB, cfg_plot, pairDir, pairTag);
    catch ME
