...
%   (bypassing overlap_pair / overlap_extract_voxel_info / etc.),
%   then run traj_diffcorr.  Each worker holds exactly 2 family atlases.
%
...
%
% Outputs  (saved to atlas_results/<tag>/dc_sweep/dc_sweep_results.mat)
...
% ── output folder ─────────────────────────────────────────────────────────────
dcRoot = fullfile(cfg.io.out_root, cfg.io.tag, 'dc_sweep');
...
%    atlas_prepare_or_load uses Java for MD5 — must run in main process.
...
for k = 1:N
    ...
    tL = tic;
    [Stmp, ~] = atlas_prepare_or_load(families{k}, cfg, grid3);
    info = atlas_cache_get_path(families{k}, Stmp.mu, Stmp.CJ, cfg);
    cache_fpaths{k} = info.fpath;

    % Ensure dense PO trace (needed by traj_diffcorr)
...
%    to overlap_pair or overlap_extract_voxel_info.
% ════════════════════════════════════════════════════════════════════════════
ckptFile = fullfile(dcRoot, 'dc_sweep_checkpoint.mat');

if use_par
    % ── PARALLEL Phase 2 ─────────────────────────────────────────────────────
    % Reuse the existing 'local' (thread) pool from Phase 1 — identical pattern
    % to run_overlap_dc_sweep_tmax.m.  Slice SA_arr/SB_arr so MATLAB sends only
...
% 6. SAVE RESULTS
% ════════════════════════════════════════════════════════════════════════════
resultsFile = fullfile(dcRoot, 'dc_sweep_results.mat');
save(resultsFile, ...
...
csvFile = fullfile(dcRoot, 'dc_change_summary.csv');
...
%
% Replaces local_process_pair: instead of calling overlap_pair +
% overlap_extract_voxel_info + overlap_visualize_bounds +
% overlap_voxel_traj_extract on the full atlas, it calls local_traj_from_ticket
...
% Identical to the version in run_overlap_dc_sweep_tmax.m.
...
%LOCAL_TRAJ_FROM_TICKET  Reconstruct T struct for traj_diffcorr from ticket.
% Identical to the version in run_overlap_dc_sweep_tmax.m.
