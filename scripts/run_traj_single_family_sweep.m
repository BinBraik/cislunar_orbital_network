...
%           traj_*_before_dc.png    —  trajectory before DC  (with arrows)
%           traj_*_after_dc.png     —  trajectory after DC   (with arrows)
%
...
%
% Available families (from cr3bp_family_ic):
...
% ===================== OUTPUT ROOT =====================
origin_tag = sanitize_fname(famOrigin);
...
[SA, ~] = atlas_prepare_or_load(famOrigin, cfg, grid3);
...
for iT = 1:nT
    fprintf('[sweep]   [%d/%d] %s\n', iT, nT, famTargets{iT});
    [SB_cell{iT}, ~] = atlas_prepare_or_load(famTargets{iT}, cfg, grid3);
end
...
famA     = SA.name;
famB     = SB.name;
pair_tag = [sanitize_fname(famA) '_TO_' sanitize_fname(famB)];
...
try
    ...
    B = overlap_visualize_bounds(V, SA, SB, O, cfg, pair_dir, pair_tag);
    ...
    % ------------------------------------------------------------------
    T = overlap_voxel_traj_extract(SA, SB, V, B, cfg);
    ...
    % ------------------------------------------------------------------
    traj_raw = traj_build_full(T);   % before DC
    traj_dc  = traj_build_full(Tc);  % after  DC

    % ------------------------------------------------------------------
    % Step 7: visualise with departure / arrival arrows
    % ------------------------------------------------------------------
    traj_visualize_full(T,  traj_raw, SA, SB, cfg, pair_dir, [pair_tag '_before_dc']);
    traj_visualize_full(Tc, traj_dc,  SA, SB, cfg, pair_dir, [pair_tag '_after_dc']);
