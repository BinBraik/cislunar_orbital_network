...
%        a. Derive in-memory subset atlases via atlas_derive_subset.
...
%           and run traj_diffcorr. Uses sliced parfor so each worker receives
...
%
% Outputs written to OUTPUT_DIR (dc_sweep_tmax_results/):
...
% I/O paths
CHECKPOINT_FILE = fullfile(repoRoot, 'dc_sweep_tmax_results', 'sweep_dc_checkpoint.mat');
OUTPUT_DIR      = fullfile(repoRoot, 'dc_sweep_tmax_results');
...
for i = 1:N
    ...
    % Ensure dense PO trace is present (needed by traj_diffcorr, inherited by subsets)
...
%
% Same as local_compute_footprint in run_overlap_dv_tmax_sweep, but also stores
...
% Theta mirror LUT (identical to overlap_pair)
...
% Extends local_fp_rows (from run_overlap_dv_tmax_sweep) with iSeed, iHead, t_nd outputs.
...
%
% Same overlap logic as local_run_pair (run_overlap_dv_tmax_sweep) plus argmin
...
try
    ...
    % 5. DV proxy (identical formula to run_overlap_dv_tmax_sweep)
...
%LOCAL_TRAJ_FROM_TICKET  Reconstruct T struct (warm start for traj_diffcorr).
%
% Replaces overlap_pair + overlap_extract_voxel_info +
%          overlap_visualize_bounds + overlap_voxel_traj_extract
...
% (subset atlas in the sweep, full atlas in run_overlap_dc_sweep). Phase 2 must
...
sol_A   = ode113(@(t,X) cr3bp_reduced_ode(t,X,SA.CJ,SA.mu,false), ...
                  [0, t_end_A], IC_A, opts);
...
sol_B   = ode113(@(t,X) cr3bp_reduced_ode(t,X,SB.CJ,SB.mu,false), ...
...
pot_B = cr3bp_potential(seed_B_frs(1), seed_B_frs(2), SB.mu);
...
T.delta_th_rad = abs(circ_diff(T.th_A_star, T.th_B_star));

% ── DV patch and totals ───────────────────────────────────────────────────────
CJstar = min(SA.CJ, SB.CJ);
pot_c  = cr3bp_potential(xc, yc, SA.mu);
