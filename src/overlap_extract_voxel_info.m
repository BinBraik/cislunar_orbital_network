...
assert(isstruct(O) && isfield(O,'ids'), 'O must be overlap struct from overlap_pair');
...
idsB = double(atlas_rows_to_voxelid(rowsB_B, grid3));
...
fprintf('[overlap] extracted voxel metadata: %d overlap voxels\n', K);
end
