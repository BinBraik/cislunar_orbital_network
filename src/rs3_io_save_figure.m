function rs3_io_save_figure(fig, outdir, baseName, cfg, varargin)
%RS3_IO_SAVE_FIGURE  Save figure as PNG and/or FIG based on cfg.
%
% Usage:
%   rs3_io_save_figure(fig, outdir, baseName, cfg)
%   rs3_io_save_figure(fig, outdir, baseName, cfg, 'Resolution', 220)
%
% Notes:
...
try
    ...
    warning('[rs3] Failed to save PNG (%s): %s', pngPath, ME.message);
end
...
if saveFig
    ...
    try
        ...
        warning('[rs3] Failed to save FIG (%s): %s', figPath, ME.message);
    end
end
