...
%     (one vectorized rs3_circ_diff call instead of one per segment).
...
dth_segs = rs3_circ_diff(p1_mat(:,3), p0_mat(:,3)); % vectorized, (nSeg x 1)
...
for k = 1:(nPts-1)
    ...
    thj = rs3_wrapToPi(p0_mat(k,3) + f_arr * dth_segs(k));  % (nsub+1) x 1
