function cfg = atlas_cfg_defaults()
%ATLAS_CFG_DEFAULTS  Default configuration schema for the atlas pipeline.
%
% Returns
...
cfg.cache.dir           = fullfile(repo_root(), 'atlas_cache');
cfg.cache.rebuild       = false;                  % if true: ignore existing cache and recompute
cfg.cache.version_tag   = 'atlas_v1_keep_masked';  % bumped when cache schema changes
...
% ---------------- Output ----------------
cfg.io = struct();
cfg.io.out_root      = fullfile(repo_root(), 'atlas_results');
