function T = rs4_voxel_traj_extract(SA, SB, V, B, cfg)
...
% proxy in rs4_overlap_visualize_bounds.
%
% Inputs
%   SA, SB : family structs from rs3_prepare_or_load_family
%   V      : voxel metadata struct from rs4_overlap_extract_voxel_info
%   B      : bounds struct from rs4_overlap_visualize_bounds (needs B.imin)
%   cfg    : config struct
%
...
fprintf('[rs4_extract] Best voxel k=%d, vid=%d\n', k, vid);
fprintf('[rs4_extract]   center (x,y,th) = (%.6f, %.6f, %.4f deg)\n', ...
    xc, yc, rad2deg(thc));
fprintf('[rs4_extract]   DVproxy = %.3f m/s  (DVturn_A=%.3f + DVpatch_ub=%.3f + DVturn_B=%.3f)\n', ...
...
if isempty(idxA)
    error('[rs4_extract] No A-side rows found for voxel vid=%d.', vid);
end

[iSeed_A, iHead_A, halfFlag_A, t_A, delta_A] = ...
    local_argmin_dv_row(rowsA_F, idxA, SA);

fprintf('[rs4_extract] A argmin: iSeed=%d iHead=%d hf=%d t=%.5f nd (%.2f days) delta=%.5f rad\n', ...
...
th_frs  = rs3_wrapToPi(pi - th_t);                      % theta-mirror (self-inverse)
...
idsB_lo = double(rs3_rows_to_voxelid(SB.Step4.rows_FRS_lower, grid3));
idsB_up = double(rs3_rows_to_voxelid(SB.Step4.rows_FRS_upper, grid3));
idxBlo  = find(idsB_lo(1:nBlo) == vid_frs);
idxBup  = find(idsB_up(1:nBup) == vid_frs);

if isempty(idxBlo) && isempty(idxBup)
    error('[rs4_extract] No B-side rows map to inverse-mirror voxel vid_frs=%d.', vid_frs);
end

[iSeed_B, iHead_B, halfFlag_B_frs, t_B, delta_B, from_lower_B] = ...
    local_argmin_dv_row_B(SB, idxBlo, idxBup);

fprintf('[rs4_extract] B argmin: iSeed=%d iHead=%d hf_frs=%d t=%.5f nd (%.2f days) delta=%.5f rad\n', ...
...
[tA_vec, XA] = ode113( ...
    @(tt,XX) rs3_core_reduced_cr3bp_model(tt, XX, SA.CJ, SA.mu, false), ...
    [0, t_A], IC_A, odeOpts);

[tB_vec, XB_frs] = ode113( ...
    @(tt,XX) rs3_core_reduced_cr3bp_model(tt, XX, SB.CJ, SB.mu, false), ...
...
delta_th     = abs(rs3_wrapToPi(th_A_star - th_B_star));
...
fprintf('[rs4_extract] DVturn_A=%.3f  DVpatch_true=%.3f  DVturn_B=%.3f  => DVtotal=%.3f m/s\n', ...
    DV_turn_A_mps, DV_patch_mps, DV_turn_B_mps, DV_total_true_mps);
fprintf('[rs4_extract] DVproxy was %.3f m/s  => tightening: %.3f m/s\n', ...
    B.min_dvproxy, B.min_dvproxy - DV_total_true_mps);
fprintf('[rs4_extract] delta_th=%.4f deg  miss dA=%.6f nd  dB=%.6f nd\n', ...
...
if ~isempty(idxBlo)
    ...
    pot = rs3_core_cr3bp_U_and_derivs(SB.SeedsLower(iS,1), SB.SeedsLower(iS,2), SB.mu);
...
if ~isempty(idxBup)
    ...
    pot = rs3_core_cr3bp_U_and_derivs(SB.SeedsUpper(iS,1), SB.SeedsUpper(iS,2), SB.mu);
    v0  = sqrt(max(2*pot.U - SB.CJ, 0));
    dv0 = 2 * v0 * sin(abs(delt)/2);
    candidates(end+1) = struct('iSeed',iS,'iHead',iH,'halfFlag_frs',hf,'t',tv,'delta',delt,'dv',dv0);
end
