function [t, X] = rs3_core_integrate_reduced_with_fallback(X0red, tspan, CJ, mu, v2tol, optsR, opts4)
...
[tR, XR, ~, ~, ie] = ode113(@(tt,XX) rs3_core_reduced_cr3bp_model(tt, XX, CJ, mu, false), ...
                            tspan, X0red, optsR);
...
if ie_last == 4
    ...
    [t, X] = ode113(@(tt,XX) rs3_core_full4d_cr3bp_model(tt, XX, mu), ...
                    tspan, X0_4, opts4);
else
    t = tR;
    X = XR;
