...
% ---- periodic orbits ----
hPOA = local_plot_po(ax, SA, [0.15 0.45 0.80], 'Origin PO');
hPOB = local_plot_po(ax, SB, [0.80 0.20 0.15], 'Target PO');

% ---- baseline arcs ----
hBaseA = plot(ax, T.XA(:,1), T.XA(:,2), '-', ...
    'Color', [0.10 0.40 0.85], 'LineWidth', 2.0, ...
    'DisplayName', sprintf('Baseline A (%.1f d)', T.tof_A_days));
hBaseB = plot(ax, T.x_B, T.y_B, '-', ...
...
dvText = sprintf(['DV_patch_true=%.2f m/s   DV_patch_dc=%.2f m/s\n' ...
                  'DV_total_true=%.2f m/s   DV_total_dc=%.2f m/s'], ...
...
legend(ax, [hPOA hPOB hBaseA hBaseB hDCA hDCB], 'Location', 'best', 'FontSize', 8);
...
function h = local_plot_po(ax, S, rgb, dispName)
h = gobjects(0);
...
h = plot(ax, xy(:,1), xy(:,2), '--', 'Color', rgb, 'LineWidth', 1.2, ...
