...
fig = figure('Color','w', 'Name', ['Transfer Trajectory Compare ' tag], ...
             ...
             'Units','pixels', 'Position',[100 100 980 760]);
ax = axes('Parent', fig);
...
plot(ax, T.XA(:,1), T.XA(:,2), '-', ...
    'Color', [0.15 0.45 0.85], 'LineWidth', 2.0, ...
    'DisplayName', sprintf('Baseline A (%.1f d)', T.tof_A_days));
plot(ax, T.x_B, T.y_B, '-', ...
    'Color', [0.85 0.25 0.20], 'LineWidth', 2.0, ...
...
plot(ax, R.stateA_patch.x, R.stateA_patch.y, 'p', ...
    'Color', [0.00 0.20 0.55], 'MarkerFaceColor', [0.00 0.20 0.55], ...
    'MarkerSize', 9, 'DisplayName', 'DC patch state A');
plot(ax, R.stateB_patch.x, R.stateB_patch.y, 'p', ...
    'Color', [0.55 0.00 0.00], 'MarkerFaceColor', [0.55 0.00 0.00], ...
...
rs3_io_save_figure(fig, outdir, ['rs4_' safeTag '_traj_compare_dc'], cfg);
end
