...
%   entries = atlas_cache_inspect(cacheDir)
%   entries = atlas_cache_inspect(cacheDir, false)     % suppress table printout
%
...
if nargin < 1 || isempty(cacheDir)
    cacheDir = fullfile(repo_root(), 'atlas_cache');
end
if nargin < 2
    verbose = true;
end
...
if isempty(mats)
    entries = [];
    if verbose
        fprintf('[atlas_cache_inspect] No .mat files in: %s\n', cacheDir);
    end
    return;
end
...
for k = 1:N
    ...
    try
        ...
        p = atlas_cache_fingerprint_parse(fp);
        e.parsed = p;
        ...
        if numel(parts) >= 5
            ...
            full_hash   = compute_md5(config_str);
