function rs3_grid_visual_validate(grid3, cfg, outdir)
...
ylabel('dummy');
grid on;
rs3_io_save_figure(f1, outdir, 'step2_y_edges', cfg);
close(f1);
...
xlabel('x'); ylabel('y');
grid on;
rs3_io_save_figure(f2, outdir, 'step2_xy_grid_near_y0', cfg);
close(f2);
...
ylabel('dummy');
grid on;
rs3_io_save_figure(f3, outdir, 'step2_theta_seam', cfg);
close(f3);
