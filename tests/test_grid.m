...
% ---- atlas_grid_make basic properties ----
...
function test_bin_center_of_domain(testCase)
    ...
    [ix, iy, it] = atlas_bin_xyth(0, 0, 0, grid3);
...
function test_bin_out_of_bounds(testCase)
    ...
    grid3 = atlas_grid_make(cfg);
    % Way outside domain
    [ix, ~, ~] = atlas_bin_xyth(100, 100, 0, grid3);
...
function test_bin_theta_wrapping(testCase)
    ...
    [~, ~, it1] = atlas_bin_xyth(0, 0, 0.5, grid3);
    [~, ~, it2] = atlas_bin_xyth(0, 0, 0.5 + 2*pi, grid3);
...
function test_keep_mask_excludes_primaries(testCase)
    ...
    CJ = 3.1;
    Keep = atlas_keep_mask_xy(grid3, CJ, mu, cfg.sys.RE_nd, cfg.sys.RM_nd);
