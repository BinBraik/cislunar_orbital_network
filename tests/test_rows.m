...
function test_count_struct(testCase)
    r = atlas_rows_empty(50);
    r.n = uint32(25);
    verifyEqual(testCase, atlas_rows_count(r), 25);
end

function test_count_matrix(testCase)
    m = rand(10, 8);
    verifyEqual(testCase, atlas_rows_count(m), 10);
end

function test_count_empty(testCase)
    verifyEqual(testCase, atlas_rows_count(atlas_rows_empty()), 0);
end

function test_count_nonsense(testCase)
    verifyEqual(testCase, atlas_rows_count('hello'), 0);
end
